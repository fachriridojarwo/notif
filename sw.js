...
importScripts(
  "https://cdn.jsdelivr.net/gh/jakearchibald/idb@97e4e878/lib/idb.js"
);

const applicationServerPublicKey =
  "BHnmL6hhk1PlAPyFKOxq3AzTNUEl2VK7LpV58Ggz_zsno69HoVDwuFFfX5ntpQoWTL0-AdmxNwiCvZ5-LrX3s5Y";
...
self.addEventListener("push", (event) => {
  ...
  const options = {
    body: data.body,
    icon: data.icon || "https://nazgul.online/notification.png",
    badge: data.badge || "https://nazgul.online/badge.png",
    vibrate: [100, 50, 100],
    data: {
  ...
  if (event.request.url.includes("send-push")) {
    event.respondWith(
      (async () => {
        ...
        return fetch(self.location.origin + "/send-push", options);
      })()
    );
  } else {
    event.respondWith(
...
async function sendSubscriptionToServer(subscription) {
  ...
  await fetch("https://api-notif.siarem.xyz/push/create-leads", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
